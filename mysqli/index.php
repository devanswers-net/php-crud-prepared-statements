<?php
// DevAnswers.net: minimal HTML UI for the members table (mysqli CRUD)
...
// http://localhost:8000/mysqli/index.php).

require __DIR__ . '/../connect.php';

// Handle form submissions before rendering the page.
$message = '';
    ...
    if ($action === 'create') {
        ...
        if ($name === '' || $email === '') {
            ...
            try {
                ...
                $message = "Added {$name} (id {$mysqli->insert_id}).";
            } catch (mysqli_sql_exception $e) {
                // 1062 = duplicate unique key (email is UNIQUE in the schema).
        ...
        $id = (int) ($_POST['id'] ?? 0);
        $mysqli->execute_query('DELETE FROM members WHERE id = ?', [$id]);
        $message = "Deleted id {$id} ({$mysqli->affected_rows} row removed).";
    }
}

// Fetch all members for the list view.
$members = $mysqli->execute_query(
...
<head>
<meta charset="utf-8">
<title>DevAnswers Members Admin (mysqli CRUD demo)</title>
<style>
...
<h1>Members Admin <small>(DevAnswers MySQLi CRUD demo)</small></h1>
...
<p class="source">
    Companion to <a href="https://devanswers.net/php-mysqli-crud-prepared-statements/">PHP MySQLi CRUD with Prepared Statements</a>.
